...
int  get_user_move_json(char* buf ,int* folder_id,char* user,char* md5)
{
    int ret =0;
    ...
    cJSON* cFolderName = cJSON_GetObjectItem(root,"md5");
    if(cFolderName==NULL)
    {
    ...
    strcpy(md5,cFolderName->valuestring);
...
int put_user_move_mysql(int folder_id,char* md5,char* user)
{
    int ret =0;
    ...
    sprintf(sql_cmd,"UPDATE  user_file_list SET folder_id = %d WHERE user = '%s' and md5 = '%s';",folder_id,user,md5);
...
int main()
{
    ...
    while(FCGI_Accept()>=0)
    {
        ...
        else
        {
            ...
            ret = get_user_move_json(buf,&folder_id,user,md5);
            if(ret !=0)
            {
            ...
            ret = put_user_move_mysql(folder_id,md5,user);
            if(ret<0)
            {
                ret=-1;
                goto END;
